var webviewGroup = function(id, topMargin, options) {
    this.id = id;
    ...
    this._init(topMargin);
};

var proto = webviewGroup.prototype;

proto._init = function(topMargin) {
    this._initParent();
    this._initNativeView();
    this._initWebviewContexts(this.options.index, topMargin);
};
...
proto._initNativeView = function() {
    this.nativeView = new plus.nativeObj.View('__MUI_TAB_NATIVE', {
        'top': this.topMargin + 'px',//这个需要根据顶部导航及顶部选项卡高度自动调整
...
proto._initWebviewContexts = function(index, topMargin) {
    for(var len = this.items.length, i = len - 1; i >= 0; i--) {
        ...
        var webviewContext = new webviewGroupContext(id, topMargin, webviewOptions, this);
 ...
 */
var webviewGroupContext = function(id, topMargin, webviewOptions, groupContext) {
    this.id = id;
    this.topMargin = topMargin
