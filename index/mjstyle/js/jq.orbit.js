    ...
    $.fn.orbit = function(options) {
      ...
      slides.eq(activeSlide)
		...
		slides.eq((activeSlide+2)%numberSlides).css({"left":options.backendLeft4,"z-index" : numberSlides-2,"top":options.backendTop2,"width": options.backendWidth2+"px","border":options.backendBorder2});
		  ...
		  slides_div.eq((activeSlide-2)%numberSlides).css({"z-index" : numberSlides-2});
		  slides_div.eq((activeSlide-1)%numberSlides).css({"z-index" : numberSlides-1});
		  slides_div.eq((activeSlide)%numberSlides).css({"z-index" : numberSlides});
		  slides_div.eq((activeSlide+1)%numberSlides).css({"z-index" : numberSlides-1});
		  slides_div.eq((activeSlide+2)%numberSlides).css({"z-index" : numberSlides-2});

		for(act=activeSlide+3;act<numberSlides;act++){
			slides.eq(act%numberSlides).css({"z-index" : numberSlides-3,"top":options.backendTop2});
			slides_div.eq(act%numberSlides).css({"z-index" : numberSlides-3});
		}
      ...
      if(options.timer) {         	
          ...
          if(timer.length != 0) {
              var rotator = $('div.timer span.rotator'),
              	mask = $('http://www.mjstyle.cn/js/div.timer span.mask'),
...
// ==================
      ...
      if(options.directionalNav) {
          ...
          var directionalNavHTML = '<div class="slider-nav"><span class="right">next ></span><span class="left">< previous</span></div>';
          orbitWrapper.append(directionalNavHTML);
          var leftBtn = orbitWrapper.children('div.slider-nav').children('http://www.mjstyle.cn/js/span.left'),
   ...
   /* for(act = 1;act<numberSlides;act++){
      	...
      	slides.eq((activeSlide+act)%numberSlides).css({"z-index" : numberSlides-1});
			...
			if(slideDirection == "next") {
				...
				if(numberSlides>5){
					for(j=numberSlides-5;j>=1;j--){
						slides_div.eq((prevActiveSlide1-j+numberSlides)%numberSlides).css({"z-index" : numberSlides-2-j});
					}
				}
            ...
            if(slideDirection == "prev") {
				...
				if(numberSlides>5){
					if(numberSlides>5){
						slides_div.eq((prevActiveSlide1+1+numberSlides)%numberSlides).css({"z-index" : numberSlides-4});
					}
				}
